% Funktion that reads a EPA-SWMM5 binary output file (*.out).
...
function [e1,e2,e3]=readswmmout3(flag,p1,p2,p3,p4)
% readswmmout2   [e1,e2,e3]=readswmmout3(flag,p1,p2,p3,p4)
%           
%           Reads a SWMM5 binary output file (*.out). 
% [d] =  readswmmout3('open',filename,[Modelname])
...
%
% [err,res]=  readswmmout3('get',d,[fromidx,toidx])
...
%               d.Nperiods
%
% [err,res]=  readswmmout3('getidx',d,[fromidx,toidx],itemidx)
...
%                                                  small bugfix for hold on and sediment
...
%	        F. Blumensaat    16.01.2020, adaption to SWMM 51011; typo at line 155
%	        A. Figueroa      14.10.2020, adaption to SWMM 51015-TEMP
...
case 'open'
    ...
    switch nn(2)
        ...
        case 51015
            d.Version=50006; 
        otherwise
            error(['Unknown SWMM Version lala',int2str(nn(2))]);
    end;
	d.FlowUnit=nn(3);
	d.nCatch=nn(4);
	d.nNode=nn(5);
    ...
    % START/ Alejandro Figueroa - Eawag 
    if d.nWTemp==1
	d.idWTemp=cell(1,1);
       nn=fread(fid,1,'int32');
       id=fread(fid,nn,'uchar');
...
case 'getitems'
    ...
    scal=swmmunits('scal',d.FlowUnit,idxu-1,d.nPollut);
...
%     res = NaN(d.Nperiods,2);
    if d.Nperiods > timeIdxLimit
        ...
        for w = 1:nReadingCycles
            ...
            [err,yNeu]=  readswmmout3('getidx',d,[IDstartTime, IDstartTime+timeIdxLimit-1],idx);
            res = vertcat(res,yNeu);
        end %for
        %%read rest
        [err,yNeu]=  readswmmout3('getidx',d,[IDstartTime+timeIdxLimit,d.Nperiods],idx);
        res = vertcat(res,yNeu);
        y = res;
    else
        %[err,res]=readswmmout2('get',d,[1,d.Nperiods]);
        [err,y]=  readswmmout3('getidx',d,[idxt1,idxt2],idx);
    end %if
