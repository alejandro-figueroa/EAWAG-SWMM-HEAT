...
int    table_parseFileLineBinary(unsigned char* line, TTable* table, double* x, double* y);
...
//
{
    char line[MAXLINE+1]; 
    #define  NBYTE 12
    unsigned char linebinary[NBYTE];

    int  code;
    //printf("timeseriebinary %d\n", TimeSeriesBinary);
    if ( TimeSeriesBinary == 0 )
    {
        if ( table->file.file == NULL ) return FALSE;
    ...
    {
        //Read Binary File
        while (!feof(table->file.file) && fread(linebinary, NBYTE, 1, table->file.file) != NULL)
        {
            code = table_parseFileLineBinary(linebinary, table, x, y);
            //printf("Read x and yy, %f, %f\n", *x,*y);
            //if (code < 0) continue;      //skip blank & comment lines
            return code;
        }
    }
...
int  table_parseFileLineBinary(unsigned char* linebinary, TTable* table, double* x, double* y)
//
//  Input:   table = pointer to a TTable structure
//           x = pointer to a date (as decimal days)
//           y = pointer to a time series value
...
//
{
    ...
    //    * s2,
    //    * s3;
    unsigned char* tPtr;     // time as string
    unsigned char* yPtr;     // value as string
    double yy;               // value as double
    DateTime d;              // day portion of date/time value
    DateTime t;              // time portion of date/time value

    //printf("Inside table_parseFileLineBinary\n");
    // --- get 3 string tokens from line and check if its a comment
    // n = sscanf(line, "%s %s %s", s1, s2, s3);
    n = (int)linebinary[0];
    //printf("Value of n= %d\n",n);
    ...
    else if (n == 3)
    {
        ...
        s1 = &linebinary[1];
        //printf("after first \n");
        if (!datetime_binToDate(s1, &d)) return FALSE;

        // --- update last recorded calendar date
        table->lastDate = d;
        tPtr = &linebinary[1 + 4];
        yPtr = &linebinary[1 + 4 + 3];
        //tPtr = s2;
        //yPtr = s3;
    }
    else return FALSE;
    ...
    //if (getDouble(tStr, &t)) t /= 24.0;
    //else if (!datetime_strToTime(tStr, &t)) return FALSE;
    if (!datetime_binToTime(tPtr, &t)) return FALSE;
