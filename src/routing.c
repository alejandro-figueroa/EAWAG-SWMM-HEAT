...
//
{
   ...
   // --- replace old water temperature state with new state
    ...
    void  addSystemInflows(DateTime currentDate, double routingStep)
    {
        ...
        // --- initialize node inflow for quality  and temperature routing
        for (j = 0; j < Nobjects[NODE]; j++) 
        {
            Node[j].qualInflow = MAX(0.0, Node[j].newLatFlow);
            Node[j].tempInflow = MAX(0.0, Node[j].newLatFlow);
        }
    }
    ...
    for ( i = 0; i < Nobjects[NODE]; i++ )
    {
        ...
        if ( q > 0.0 )
        {
            ...
            if (TempModel.active == 1)
            {
              ...
              //  fprintf(stdout, " removeOutflows 0 %f, %f, %f \n", w, q, Node[i].newTemp);
        ...
        if ( q < 0.0 )
        {
            ...
            {
                w = -q * Node[i].newTemp;
               // fprintf(stdout, " removeOutflows 1 %f, %f, %f \n", w,q,Node[i].newTemp);
