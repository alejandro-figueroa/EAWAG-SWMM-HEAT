...
//
{
    ...
    /* START modification by Alejandro Figueroa | EAWAG */
    //printf("save id TEMP %d\n", TempModel.active);
    if (TempModel.active == 1) 
    {
        //printf("inside id TEMP\n");
        output_saveID(WTemperature.ID, Fout.file);
    }
    // printf("out save id TEMP\n");
    /* END modification by Alejandro Figueroa | EAWAG */

    InputStartPos = ftell(Fout.file);

    //printf("save subcatch info\n");
    ...
    //if ( Fout.mode == SCRATCH_FILE ) output_checkFileSize(); after 5.2 release
    //printf("end error code output %d\n", ErrorCode);
    return ErrorCode;
}
