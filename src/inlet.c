...
// inlet_adjustTempInflows   called by routing_execute
// inlet_adjustTempOutflows  called by routing execute
// inlet_writeStatsReport    called by statsrpt_writeReport
// inlet_capturedFlow        called by findLinkMassFlow in qualrout.c and temperature.c
...
void  inlet_adjustTempInflows()
//
//  Input:   none
//  Output:  none
//  Purpose: adjusts accumulated temperature inflows at each
//           inlet's bypass and capture nodes after a flow routing step has
//           been taken prior to a temperature routing step. 
//
{
    int     i, j, m;
    double qNet;
    ...
    if (!UsesInlets) return;
    if (IgnoreWTemperature || TempModel.active == 0) return;
    for (inlet = FirstInlet; inlet != NULL; inlet = inlet->nextInlet)
    {
        // --- identify indexes of inlet's bypass (j) and capture (m) nodes 
        i = inlet->linkIndex;
        ...
        {
            // --- add net capture flow to capture node's accumulated flow
            //     inflow for temperature routing
            Node[m].tempInflow += qNet;

            // --- and do the same for temperature mass flows
            //     (Node[m].newTemp is the mass inflow accumulator for node m)
                Node[m].newTemp += qNet * Node[j].oldTemp;
        }

        // --- there's a net backflow from the capture to the bypass node
        else
        {
            // --- add the backflow flow rate and temperature mass flow to the
            //     bypass node's accumulated flow and temperature mass inflow
            qNet = -qNet;
            Node[j].tempInflow += qNet;
            Node[j].newTemp += qNet * Node[m].oldTemp;
        }
    }
}
/* END modification by Alejandro Figueroa | EAWAG */
...
//
{
    ...
    // --- these variables, declared in massbal.c, accumulate system-wide flow and
    //     temperature mass fluxes over a time step to use in mass balances
    extern TRoutingTotals StepFlowTotals;
    extern TRoutingTotals StepTempTotals;
    ...
    for (j = 0; j < Nobjects[NODE]; j++)
    {
        ...
        if (Node[j].inlet == CAPTURE)
        {
            ...
            if (q > 0.0)
            {
                ...
                // --- also remove temperature overflow mass from system totals
                if (!IgnoreWTemperature || TempModel.active == 1)
                    //for (p = 0; p < Nobjects[POLLUT]; p++)
                    //{
                        w = q * Node[j].newTemp;
                        StepTempTotals.flooding -= w;
                    //}
            }
        }
    }
        ...
        for (inlet = FirstInlet; inlet != NULL; inlet = inlet->nextInlet)
        {
           ...
           //      node having a net negative lateral inflow
            ...
            if (q > 0.0 && Node[j].newLatFlow < 0.0)
                ...
                // --- remove the temperature mass in the captured flow from
                //     the system totals since it does not leave the system
                //     (it is sent to the inlet's capture node)
               // for (p = 0; p < Nobjects[POLLUT]; p++)
                //{
                    w = q * Node[j].newTemp;
                    StepTempTotals.outflow -= w;
                //}
        }
    }
}
