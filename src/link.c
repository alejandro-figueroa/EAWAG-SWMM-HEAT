...
//
{
    ...
    Link[j].oldHeat = 0.0;
    Link[j].newHeat = 0.0;
    Link[j].oldsedtemp = NAN;
    Link[j].newsedtemp = NAN;
...
//=============================================================================
 ...
 //
 {
        ...
        Link[j].newTemp = 0.0;
        Link[j].oldHeat = Link[j].oldHeat + Link[j].newHeat;
        Link[j].newHeat = 0.0;
        Link[j].oldsedtemp = Link[j].newsedtemp;
        Link[j].newsedtemp = 0.0;
...
//
{
    ...
    double y,                     // depth
        ...
        u,                     // velocity
        v,                     // volume
        c, c1,c2,              // capacity, setting or concentration
     area,                     // Link Area
    width,                     // top width
 ...
 dryPerim,                     // Dry Perimeter
    ...
    if (!IgnoreWTemperature && TempModel.active == 1)
    {
        ...
        c1 = f1 * Link[j].oldHeat + f * Link[j].newHeat;
        c2 = f1 * Link[j].oldsedtemp + f * Link[j].newsedtemp;

        x[LINK_QUAL + Nobjects[POLLUT]]     = (float)c ;
        x[LINK_QUAL + Nobjects[POLLUT] + 1] = (float)c1;
        x[LINK_QUAL + Nobjects[POLLUT] + 2] = (float)c2;
    }
    /* END modification by Alejandro Figueroa | Eawag */
}
